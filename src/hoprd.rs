...
use k8s_openapi::api::core::v1::{Container, ContainerPort, PodSpec, PodTemplateSpec, Volume, Probe, EnvVar, EnvVarSource, SecretKeySelector, HTTPGetAction, SecretVolumeSource, KeyToPath, EmptyDirVolumeSource, VolumeMount, ResourceRequirements, Service, ServiceSpec, ServicePort};
...
use crate::crd::{HoprdSpec, Resource};
...
pub async fn create_deployment(
    ...
    labels.insert("app.kubernetes.io/name".to_owned(), "hoprd".to_owned());
    labels.insert("app.kubernetes.io/instance".to_owned(), name.to_owned());

    let mut image = String::from("gcr.io/hoprassociation/hoprd:");
    image.push_str(&hoprd_spec.version);
...
async fn build_resource_requirements(resources: &Option<Resource>) -> Option<ResourceRequirements> {
    ...
    if resources.is_some() {
        ...
        value_limits.insert("memory".to_owned(), Quantity(resource.limits.memory.to_owned()));
        value_requests.insert("cpu".to_owned(), Quantity(resource.requests.cpu.to_owned()));
        value_requests.insert("memory".to_owned(), Quantity(resource.requests.memory.to_owned()));
    } else {
    ...
    volumes.push(Volume {       
        ...
        secret: Some(SecretVolumeSource {
            ...
            items: Some(vec![KeyToPath {
                    key: hoprd_spec.secret.identity_ref_key.as_ref().unwrap_or(&"HOPRD_IDENTITY".to_owned()).to_owned(),
                    mode: Some(440),
...
async fn build_secret_env_var(hoprd_spec: &HoprdSpec) -> Vec<EnvVar> {
    ...
    env_vars.push(EnvVar {
        name:"HOPRD_PASSWORD".to_owned(),
        value_from: Some(EnvVarSource {
            secret_key_ref: Some(SecretKeySelector {
                key: hoprd_spec.secret.password_ref_key.as_ref().unwrap_or(&"HOPRD_PASSWORD".to_owned()).to_string(),
     ...
     env_vars.push(EnvVar {
        name:"HOPRD_API_TOKEN".to_owned(),
        value_from: Some(EnvVarSource {
            secret_key_ref: Some(SecretKeySelector {
                key: hoprd_spec.secret.api_token_ref_key.as_ref().unwrap_or(&"HOPRD_API_TOKEN".to_owned()).to_string(),
                name: Some(hoprd_spec.secret.secret_name.to_owned()),
                ..SecretKeySelector::default()
            }),
            ..EnvVarSource::default()
...
async fn build_crd_env_var(hoprd_spec: &HoprdSpec) -> Vec<EnvVar> {
    ...
    let mut env_vars = Vec::with_capacity(1);
    env_vars.push(EnvVar {
        name:"HOPRD_ENVIRONMENT".to_owned(),
     ...
     if hoprd_spec.default_strategy.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_DEFAULT_STRATEGY".to_owned(),
     ...
     if hoprd_spec.max_auto_channels.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_MAX_AUTOCHANNELS".to_owned(),
     ...
     if hoprd_spec.auto_redeem_tickets.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_AUTO_REDEEM_TICKETS".to_owned(),
     ...
     if hoprd_spec.check_unrealized_balance.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_CHECK_UNREALIZED_BALANCE".to_owned(),
            value:Some(hoprd_spec.check_unrealized_balance.as_ref().unwrap().to_string()),
            ..EnvVar::default()
         });
     }
     ...
     if hoprd_spec.allow_private_node_connections.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_ALLOW_PRIVATE_NODE_CONNECTIONS".to_owned(),
            value:Some(hoprd_spec.allow_private_node_connections.as_ref().unwrap().to_string()),
            ..EnvVar::default()
         });
     }
     ...
     if hoprd_spec.test_announce_local_address.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_TEST_ANNOUNCE_LOCAL_ADDRESSES".to_owned(),
            value:Some(hoprd_spec.test_announce_local_address.as_ref().unwrap().to_string()),
            ..EnvVar::default()
         });
     }
     ...
     if hoprd_spec.heartbeat_interval.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_HEARTBEAT_INTERVAL".to_owned(),
     ...
     if hoprd_spec.heartbeat_threshold.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_HEARTBEAT_THRESHOLD".to_owned(),
     ...
     if hoprd_spec.heartbeat_variance.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_HEARTBEAT_VARIANCE".to_owned(),
     ...
     if hoprd_spec.on_chain_confirmations.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_ON_CHAIN_CONFIRMATIONS".to_owned(),
            value:Some(hoprd_spec.on_chain_confirmations.as_ref().unwrap().to_string()),
            ..EnvVar::default()
         });
     }
     ...
     if hoprd_spec.network_quality_threshold.is_some() {
        env_vars.push(EnvVar {
            name:"HOPRD_NETWORK_QUALITY_THRESHOLD".to_owned(),
            value:Some(hoprd_spec.network_quality_threshold.as_ref().unwrap().to_string()),
            ..EnvVar::default()
         });
     }
...
async fn build_default_env_var() -> Vec<EnvVar> {
    ...
    env_vars.push(EnvVar {
     ...
     env_vars.push(EnvVar {
        name:"HOPRD_IDENTITY".to_owned(),
        value:Some("/app/hoprd-identity/.hopr-id".to_owned()),
        ..EnvVar::default()
     });
     env_vars.push(EnvVar {
        name:"HOPRD_DATA".to_owned(),
        value:Some("/app/hoprd-db".to_owned()),
        ..EnvVar::default()
     });
     env_vars.push(EnvVar {
        name:"HOPRD_API".to_owned(),
        value:Some("true".to_owned()),
        ..EnvVar::default()
     });
     env_vars.push(EnvVar {
        name:"HOPRD_API_HOST".to_owned(),
        value:Some("0.0.0.0".to_owned()),
        ..EnvVar::default()
     });
     env_vars.push(EnvVar {
        name:"HOPRD_INIT".to_owned(),
        value:Some("true".to_owned()),
        ..EnvVar::default()
     });
     env_vars.push(EnvVar {
        name:"HOPRD_HEALTH_CHECK".to_owned(),
        value:Some("true".to_owned()),
        ..EnvVar::default()
     });
     env_vars.push(EnvVar {
        name:"HOPRD_HEALTH_CHECK_HOST".to_owned(),
        value:Some("0.0.0.0".to_owned()),
        ..EnvVar::default()
     });
     return env_vars
